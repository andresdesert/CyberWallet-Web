...
import { mockLoginResponse, mockRegisterResponse } from '@/mocks/authMock';
...
export const register = async (userData: RegisterRequest): Promise<RegisterResponse> => {
  try {
    ...
    // Devolver respuesta simulada con algunos datos personalizados
    return {
      ...mockRegisterResponse,
      alias: `${userData.username || 'user'}.${Math.floor(Math.random() * 1000)}`,
    };
